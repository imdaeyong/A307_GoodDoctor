...
public interface HospitalDao extends JpaRepository<Hospital, String> {
	...
	@Query(value = "select * from hospital where subject = ?1 order by POW(lng-?2,2) + POW(lat-?3,2), sido,gu limit ?4 offset ?5", nativeQuery = true)
	List<Hospital> selectHospitalSubjectLimitOffset(String subject,double lon, double lat,int limit, int offset);
