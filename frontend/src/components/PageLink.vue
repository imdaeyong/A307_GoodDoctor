...
export default {
  ...
  methods: {
    ...
    initComponent(){
      // console.log("!!!!!!!!!!!!!initcomponent :  " + this.subject + " " + this.sido + " " + this.gu);
      axios
        .get(`http://localhost:8080/hospitals/pagelink/count?subject=${this.subject}&sido=${this.sido}&gu=${this.gu}`)
        .then(({data}) => {
